  ...
  <div class="modal fade" id="creditsmodal" aria-hidden="true">
   ...
   <div class="form-group row">
                  ...
                  <div class="col-sm-7">
                    ...
                    ?>
                    <option value="<?php echo $row["empID"]; ?>"><?php echo $row["firstname"] . " " . $row["middlename"] . " " . $row["lastname"] ; ?></option>
                    <?php }
                      } else {
...
<table border="1">  
   ...
   <?php
                            ...
                            if(mysqli_num_rows($query_run) > 0)
                            {
                                ...
                                {
                                    ?>
                                    <input type="checkbox" name="techno[]" value="<?php echo $brand['id']; ?>" /> <?php echo $brand['leavetitle']; ?> <br/>
                                    <?php
                                }
                            }
                            else
...
$connect=mysqli_connect("localhost", "root", "","db_wblms")or die("cannot connect");//connection string  
...
foreach($checkbox1 as $chk1)  
   {  
      //$chk .= ",".$chk1;  
    $query= mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM tbl_leave_list WHERE id='$chk1'"));
    $hrs = $query['hours'];
    $mins = $query['minutes'];
    echo'<script>console.log('.$chk1.');</script>';  
    //$ins = mysqli_query($con,"INSERT INTO tbl_leavecredits(emp_id,leave_id,hrs_remaning,mins_remaining)");
    $in_ch=mysqli_query($con,"INSERT INTO tbl_leavecredits(emp_id,leave_id,hrs_remaining,mins_remaining) VALUES('$employeename','$chk1','$hrs','$mins')"); 
    /*$insert_que = mysqli_query($con,"INSERT INTO tbl_l");*/
   }  
...
if($in_ch)  
   ...
   {  
      echo'<script>alert("Failed To Insert");console.log('.$checkbox1.');</script>';  
   }  
}  
?>  
